...
1.  **自動化資料抓取 (Auto Fetching)**:
    ...
    - 自動生成年份索引 (`docs/opendata/holiday/years.json`)。
2.  **RESTful API**:
    - `GET /api/holidays/{year}`: 取得指定年份的假日資料。
3.  **Web Visualization**:
    - 內建靜態網頁 (`docs/index.html`)，可直接瀏覽解析後的假日資料。
    - 支援 GitHub Pages 部署 (資料與頁面皆位於 `docs/` 目錄)。
...
- `docs/`: 靜態資源輸出目錄 (包含 index.html 與生成的 JSON)，可直接用於 GitHub Pages。
